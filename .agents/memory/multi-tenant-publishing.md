...
The platform is intended for many users: email login identifies the user and workspace, while each destination channel is connected separately and owned by that user. Telegram publishing must use bot membership/admin permission checks; never treat the email login session as Telegram publishing authorization.
